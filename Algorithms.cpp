...
#include <stack> // Include stack header for DFS implementation
#include <queue>
#include <vector>
#include <string>
...
namespace Algorithms {
    ...
    int isConnected(Graph graph) {
        const std::vector<std::vector<int>> g = graph.setMatrix();
        int n = g.size();
        if (n == 0) return 1;
     ...
     */
    std::string isBipartite(Graph graph) {
        const std::vector<std::vector<int>> g = graph.setMatrix();
        int n = g.size();
